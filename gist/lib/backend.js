...
// Set once the backend is deployed for review (see reading-backend/DEPLOY.md).
...
// reading-backend project on disk. This is a personal-machine dev fallback,
...
const CANDIDATE_BACKEND_DIRS = [
  path.join(__dirname, "..", "..", "reading-backend"),
  path.join(process.resourcesPath || "", "reading-backend"),
  "/Users/macbookvonluis/Documents/Code/timeaware/reading-backend",
];
...
/**
 * Checks whether something is already answering /api/health at the given
...
function isAlive(baseUrl = BASE_URL, timeout = 1200) {
  return new Promise((resolve) => {
    ...
    const req = client.get(`${baseUrl}/api/health`, { timeout }, (res) => {
      res.resume();
...
/**
 * Ensures a reading-backend is reachable. A hosted URL (GIST_BACKEND_URL, or
...
async function ensureBackend() {
  ...
  if (!fs.existsSync(UVICORN_BIN)) {
    return {
      baseUrl: BASE_URL,
      spawned: false,
      error: `No reading-backend virtualenv found at ${UVICORN_BIN}. Run "python3.11 -m venv .venv && .venv/bin/pip install -r requirements.txt" inside reading-backend/ first.`,
    };
  }
  ...
  return {
    baseUrl: BASE_URL,
    spawned: true,
    error: ok ? null : "reading-backend did not become healthy within 15s",
  };
}
