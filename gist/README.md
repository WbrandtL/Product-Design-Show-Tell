...
## Schema fix: the renderer never matched the real backend

This app (and the browser extension it briefly shipped alongside) was
originally built against an imagined backend contract - a `figures[]` array,
uppercase forms (`SPECTRUM`/`QUADRANT`/`COMPARISON`/`PROCESS`/`CONCEPT_MAP`),
a `NO_FIGURE` state, and node fields (`body`/`attribution`/`hedge`/
`off_axis`/`placement`) that don't exist in `gist-backend/app/schema.py`.
That contract was never actually implemented on the backend - the real
`ExplainResponse` is a single flat figure per response (`layout_hint` one of
`comparison_table`/`flow`/`quadrant`/`hierarchy`/`timeline`, nodes carrying
`label`/`kind`/`one_line`/`emphasis`/`quote`), with no `figures` wrapper and
no "no figure" state (every valid response has 2-7 nodes).

Because `renderer/modal.js` checked `response.figures.length > 0` - always
`undefined.length`, i.e. always false - every single capture rendered as the
"NO FIGURE" fallback card, regardless of what the backend actually returned.
`renderer/diagrams.js` and `renderer/modal.js` have been rewritten to draw
from the real schema instead: `layout_hint` dispatches to one of three
renderers (`comparison_table` → side-by-side cards with edge notes, `flow`/
`timeline` → a left-to-right chain, `hierarchy`/`quadrant` → a radial node
graph, since the backend supplies no axis/placement data for a true 2D
plot), and a node's `quote` surviving span verification is what drives the
verified-vs-unattributed (solid vs. dashed) mark, replacing the old
`attribution.named` check.

Verified: `renderFigureDiagram` produces real diagram markup (SVG + cards,
not the "Unknown layout" fallback) for all three of `gist-backend`'s live
sample fixtures - `trust.json` (comparison_table), `climate_feedback.json`
(flow), `method_taxonomy.json` (hierarchy) - and a live `/v1/explain` round
trip against an arbitrary (non-sample) passage in fixture mode correctly
degrades to dashed/unverified cards rather than silently falling back to
"no figure". Not re-verified inside the running Electron app by clicking
through a real system-wide capture, since that requires granting
Accessibility permission interactively.
