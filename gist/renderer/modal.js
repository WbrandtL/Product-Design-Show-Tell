...
function render() {
  ...
  const hasFigure = rec.response && rec.response.figures && rec.response.figures.length > 0;
  const isNoFigure = rec.response && !hasFigure;

  if (hasFigure) {
    const fig = rec.response.figures[0];
    chFormEl.textContent = fig.form;
    chBasisEl.textContent = rec.response.domain || "";
    cardBodyEl.innerHTML = renderFigureDiagram(fig);
    actQuestionEl.textContent = fig.question;
    btnRegenerate.classList.remove("disabled");
    btnCopy.classList.remove("disabled");
    btnDownload.classList.remove("disabled");
  } else if (isNoFigure) {
    chFormEl.textContent = "no figure";
    chBasisEl.textContent = rec.response.domain || "";
    cardBodyEl.innerHTML = renderNoFigure(rec.response);
