...
the local `reading-backend`. Results are stored in a small on-device library
...
The backend must already be set up (see `../reading-backend/README.md` - venv
...
On launch it looks for a reading-backend already running on port 8731 and, if
none is found, spawns `../reading-backend/.venv/bin/uvicorn` itself. It shuts
...
still find `reading-backend` via the absolute fallback path in
`lib/backend.js` regardless of where the `.app` itself lives, as long as
`reading-backend` stays where it is on this machine - see the note in that
...
`../reading-backend/README.md` for why).
...
`../reading-backend/PROMPT_SPEC.md` ("figure-selection") - see that file's
