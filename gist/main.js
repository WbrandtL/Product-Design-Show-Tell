 ...
 * @param {string} pathName the request path, e.g. "/v1/explain"
...
async function processQueue() {
  ...
  try {
    try {
      const { status, json } = await postJson("/v1/explain", { passage: text });
      if (status === 200) {
    ...
    if (record) {
      ...
      const notif = record.response
        ? new Notification({ title: record.response.takeaway, body: "Click the floating icon to view it." })
...
app.whenReady().then(async () => {
  ...
  ipcMain.handle("regenerate", async (_e, id) => {
    ...
    await backendReadyPromise;
    try {
      const { status: httpStatus, json } = await postJson("/v1/explain", { passage: rec.passage, force: true });
      if (httpStatus === 200) {
